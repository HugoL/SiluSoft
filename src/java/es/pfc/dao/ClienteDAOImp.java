...
public class ClienteDAOImp implements ClienteDAO {
    ...
    return (alResultado);
     ...
     public List listalf(int idCentro, String letra)throws Exception{
        ...
        List list = new ArrayList();
        //Vector vector =new Vector();
        String sql="SELECT DISTINCT `Clientes`.`IdCliente`,`Clientes`.`Dni`,`Clientes`.`Nombre`, `Clientes`.`Apellidos`, `Clientes`.`Apellido2` FROM `Clientes`, `Clientes-Centros`, `Usuarios`WHERE `Clientes-Centros`.`IdCliente`=`Clientes`.`IdCliente` AND `Clientes-Centros`.`IdCentro`="+idCentro+" AND apellidos LIKE '"+letra+"%'ORDER BY Apellidos;";
        try {   
			statement = conn.prepareStatement(sql); 
                        result=statement.executeQuery();
